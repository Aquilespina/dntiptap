...
export function DropDownList({ editor }: DropDownListProps) {
    ...
    const getItemStyle = (type: "bulletList" | "orderedList") =>
        `flex items-center gap-2 px-3 py-2 text-sm transition-colors ${isActive(type) ? "bg-gray-100 font-semibold" : "hover:bg-gray-100"
        }`;
    ...
    return (
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <button
                    type="button"
                    className={`rounded p-1 transition hover:bg-gray-200 ${isAnyActive ? "bg-gray-300" : ""
                        }`}
                    title="Lista"
                    aria-label="Lista"
                >
